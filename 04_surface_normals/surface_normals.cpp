...
{
    const std::optional<T> may_hit_sphere { hit_sphere(
        kSphereCenter<T>, kSphereRadius<T>, r) };

    if (may_hit_sphere) {
        const point3<T> hit_point { r.at(*may_hit_sphere) };
        const vec3<T> normal { hit_point - kSphereCenter<T> };
        return color<T> { T(0.5) * normal.x() + T(0.5),
            T(0.5) * normal.y() + T(0.5), T(0.5) * normal.z() + T(0.5) };
    }
    const vec3<T> unit_direction { unit_vector(r.direction()) };
    const T a { T(0.5) * (unit_direction.y() + T(1)) };

    return (T(1) - a) * color<T> { T(1), T(1), T(1) }
    + a * color<T> { T(0.5), T(0.7), T(1) };
}
...
int main()
{
    ...
    constexpr std::int32_t image_height { static_cast<std::int32_t>(
    ...
    const vec3d pixel_delta_v { viewport_v
    ...
    const point3d viewport_upper_left { camera_center
        - vec3d(0, 0, focal_length) - viewport_u / 2.0 - viewport_v / 2.0 };
