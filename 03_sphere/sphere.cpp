...
template <std::floating_point T>
constexpr auto kSphereRadius = static_cast<T>(0.5);
...
color<T> ray_color(const ray<T>& r)
{
    ...
    return (T { 1 } - a) * color<T> { T { 1.0 }, T { 1.0 }, T { 1.0 } } + a * color<T> { T { 0.5 }, T { 0.7 }, T { 1.0 } };
}
...
int main()
{
    ...
    const point3d viewport_upper_left = camera_center - vec3d(0, 0, focal_length) // The viewport is focal_length away from the camera, in the -z direction
        - viewport_u / 2.0                                                        // the viewport is to the left of the camera
        - viewport_v / 2.0;                                                       // the view port is to the top of the camera

    const point3d pixel00_loc = viewport_upper_left + 0.5 * (pixel_delta_u + pixel_delta_v); // half a pixel away from the corner

    // Render
    std::cout << std::format("P3\n{} {}\n255\n", image_width, image_height);
    for (std::size_t row { 0 }; row < image_height; ++row) {
        for (std::size_t col { 0 }; col < image_width; ++col) {
            const point3d pixel_center = pixel00_loc + (static_cast<double>(col) * pixel_delta_u) + (static_cast<double>(row) * pixel_delta_v);
